...
ToText.IMPLEMENTED[constants.DAILY]   = common;
ToText.IMPLEMENTED[constants.WEEKLY]  = common;
ToText.IMPLEMENTED[constants.MONTHLY] = common;
ToText.IMPLEMENTED[constants.YEARLY]  = ['byweekno', 'byyearday'].concat(common);
...
ToText.prototype = {
    ...
    toString: function() {
        ...
        this[constants.FREQUENCIES[this.options.freq]]();
...
var parseText = function(text, language) {
    ...
    function S() {
        ...
        case 'month(s)':
            options.freq = constants.MONTHLY;
            if (ttr.nextSymbol()) {
                ON();
                F();
            }
        ...
        case 'sunday':
            options.freq = constants.WEEKLY;
            options.byweekday = [constants[ttr.symbol.substr(0, 2).toUpperCase()]];
            ...
            while (ttr.accept('comma')) {
                ...
                options.byweekday.push(constants[wkd]);
                ttr.nextSymbol();
            }
            MDAYs();
            F();
        ...
        do {
            ...
            if(nth = decodeNTH()) {
                ...
                if (wkd = decodeWKD()) {
                    ...
                    options.byweekday.push(constants[wkd].nth(nth));
                } else {
                ...
                if(!options.byweekday)
                    options.byweekday = [];
                options.byweekday.push(constants.MO);
                options.byweekday.push(constants.TU);
                options.byweekday.push(constants.WE);
                options.byweekday.push(constants.TH);
                options.byweekday.push(constants.FR);
